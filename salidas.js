...
function render(){
  ...
  target.innerHTML=`<main class="qf-rec-page"><div class="qf-rec-titlebar"><div><h1>Salidas</h1><p>Despacho de productos, guías de remisión y control de transporte.</p></div><button class="primary qf-rec-new" type="button">+ Nueva salida</button></div>
    ...
    ${shipments.length?`<div class="qf-rec-table-wrap"><table class="qf-rec-table"><thead><tr><th>Fecha</th><th>Guía</th><th>Cliente</th><th>Destino</th><th>Peso</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>${shipments.map(x=>{const c=customers.find(y=>y.id===x.customer_id);return `<tr><td>${esc(x.guide_date)}</td><td><b>${esc(x.guide_number||'Sin guía')}</b></td><td>${esc(c?.business_name||'')}</td><td>${esc(x.destination||'—')}</td><td>${num(x.gross_weight_kg).toLocaleString('es-PE')} kg</td><td><span class="qf-rec-status">${esc(x.status||'')}</span></td><td><button class="secondary qf-rec-edit" type="button" data-id="${esc(x.id)}">Ver / editar</button> <button class="secondary qf-rec-del" type="button" data-id="${esc(x.id)}">Eliminar</button></td></tr>`;}).join('')}</tbody></table></div>`:`<div class="qf-rec-empty"><div class="qf-rec-empty-icon">📤</div><h3>Aún no hay salidas</h3><p>Registra aquí las guías con las que los productos salen de la planta.</p><button class="primary qf-rec-first" type="button">+ Registrar primera salida</button></div>`}
    </section></main>`;
...
function openForm(id=null){
  ...
  <form id="qfShipForm"><div class="qf-rec-section"><h3>Documento y cliente</h3><div class="qf-rec-grid">
    <label>Fecha<input name="guide_date" type="date" required value="${esc(x?.guide_date||today())}"></label><label>Hora<input name="guide_time" type="time" value="${esc((x?.guide_time||'').slice(0,5))}"></label><label>Tipo<select name="document_type"><option selected>Guía de Remisión</option><option>Factura</option><option>Otro</option></select></label>
    <label>Serie<input name="guide_series" placeholder="E001" value="${esc(parts.series)}"></label><label>Número<input name="guide_number" required placeholder="00001234" value="${esc(parts.number)}"></label><label>Orden de compra<select name="purchase_order_id"><option value="">— Seleccionar OC —</option>${orders.map(o=>`<option value="${esc(o.id)}" ${x?.purchase_order_id===o.id?'selected':''}>${esc(o.oc_number)}${o.operating_unit?' · '+esc(o.operating_unit):''}</option>`).join('')}</select></label>
...
async function save(event,id,modal,lines){
  ...
  if(!number){alert('Ingresa el número de guía.');return;}if(!String(form.get('customer_name')||'').trim()){alert('Ingresa el cliente.');return;}if(!items.length||items.some(v=>v.quantity<=0)){alert('Cada material debe tener una cantidad mayor que cero.');return;}
