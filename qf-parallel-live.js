...
if (new URLSearchParams(location.search).get('parallel') === '1') {
  ...
  if (!document.getElementById('qpl-style')) {
    ...
    style.textContent = `
      ...
      .qpl-wrap{padding:8px 0 24px}.qpl-head{display:flex;justify-content:space-between;gap:16px;align-items:flex-start;flex-wrap:wrap;margin-bottom:18px}
      .qpl-head h1{margin:0 0 5px}.qpl-head p{margin:0;opacity:.7}
      .qpl-badge{border:1px solid #087f4f;border-radius:999px;padding:8px 12px;font-size:11px;font-weight:800;white-space:nowrap}
      .qpl-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:14px}.qpl-grid>div{border:1px solid #ddd;border-radius:12px;padding:14px;background:rgba(255,255,255,.35)}
      .qpl-grid span{display:block;font-size:11px;text-transform:uppercase;opacity:.6}.qpl-grid b{display:block;margin-top:3px;font-size:22px}
      .qpl-toolbar{display:flex;gap:10px;margin:12px 0}.qpl-toolbar input{flex:1;min-width:180px;padding:10px;border:1px solid #ccc;border-radius:9px;background:inherit;color:inherit}.qpl-toolbar button{padding:9px 13px;border:1px solid #ccc;border-radius:9px;background:inherit;color:inherit;cursor:pointer}
      .qpl-table{overflow:auto;border:1px solid #ddd;border-radius:12px;background:rgba(255,255,255,.3)}.qpl-table table{width:100%;border-collapse:collapse;min-width:760px}
      .qpl-table th,.qpl-table td{padding:10px 12px;border-bottom:1px solid #eee;text-align:left;font-size:13px}.qpl-table th{font-size:11px;text-transform:uppercase;opacity:.65}
      .qpl-empty{padding:28px;text-align:center;opacity:.65;border:1px dashed #ccc;border-radius:12px}.qpl-note{font-size:12px;opacity:.6}
      @media(max-width:800px){.qpl-grid{grid-template-columns:repeat(2,1fr)}}@media(max-width:500px){.qpl-grid{grid-template-columns:1fr}.qpl-toolbar{flex-wrap:wrap}}
    `;
  ...
  function hideOperationalButtons() {
    ...
    [...nav.children].forEach(button => {
      if (button.classList.contains('qpl-nav')) return;
  ...
  async function load(type) {
    ...
    // Las tablas operativas no tienen user_id: consulta directa, sin filtro por usuario.
  ...
  function renderTable(type, rows, search = '') {
    const filtered = search ? rows.filter(row => norm(JSON.stringify(row)).includes(norm(search))) : rows;
    if (!filtered.length) return '<div class="qpl-empty">No hay registros para mostrar.</div>';

    let cols = columns[type].filter(([, keys]) => filtered.some(row => keys.some(key => row?.[key] !== null && row?.[key] !== undefined && row?.[key] !== '')));
    if (cols.length < 4) {
      cols = Object.keys(filtered[0]).filter(key => !['id', 'user_id', 'created_at', 'updated_at'].includes(key)).slice(0, 10).map(key => [key, [key]]);
    }

    return `
      <div class="qpl-table"><table><thead><tr>${cols.map(([label]) => `<th>${esc(label)}</th>`).join('')}</tr></thead>
      <tbody>${filtered.map(row => `<tr>${cols.map(([, keys]) => `<td>${esc(first(row, keys))}</td>`).join('')}</tr>`).join('')}</tbody></table></div>
  ...
  async function open(type) {
    ...
    content.innerHTML = `
      <main class="qpl-wrap"><div class="qpl-head"><div><h1>${esc(title)}</h1><p>Consulta operativa desde el Dashboard Administrador.</p></div><span class="qpl-badge">● SOLO LECTURA</span></div>
      <div id="qpl-body"><div class="qpl-empty">Cargando ${esc(title)}…</div></div></main>
    `;
    ...
    try {
      ...
      const low = type === 'inventario' ? rows.filter(row => {
      ...
      body.innerHTML = `
        ...
        <div class="qpl-toolbar"><input id="qpl-search" type="search" placeholder="Buscar en ${esc(title)}…" autocomplete="off"><button id="qpl-refresh" type="button">↻ Actualizar</button></div>
        <div id="qpl-table">${renderTable(type, rows)}</div>
      `;

      document.querySelector('#qpl-search')?.addEventListener('input', event => {
        const table = document.querySelector('#qpl-table');
        if (table) table.innerHTML = renderTable(type, rows, event.target.value);
      });
      document.querySelector('#qpl-refresh')?.addEventListener('click', () => open(type));
    } catch (error) {
      const body = document.querySelector('#qpl-body');
      if (body) body.innerHTML = `<div class="qpl-empty">No fue posible consultar ${esc(title)}. ${esc(error?.message || 'Error de conexión.')}</div>`;
    }
  }
  ...
  function ensureNav() {
    ...
    // No reconstruir si los tres botones ya están presentes: evita ciclos con MutationObserver.
    const present = [...box.querySelectorAll('[data-qpl]')].map(button => button.dataset.qpl);
    if (views.every(([id]) => present.includes(id))) return;

    box.innerHTML = `
      <p class="qpl-nav-label">CONSULTA OPERATIVA</p>
      ${views.map(([id, icon, label]) => `<button type="button" class="qpl-nav-btn" data-qpl="${id}"><span>${icon}</span><span>${label}</span><small>Consulta</small></button>`).join('')}
    `;
    ...
    box.querySelectorAll('[data-qpl]').forEach(button => {
      button.addEventListener('click', event => {
        ...
        box.querySelectorAll('[data-qpl]').forEach(item => item.classList.remove('active'));
